...
// Busy reg
wire cont_flag_enable, cont_flag_i, cont_flag_o;
...
//Testing (hgi)
assign cont_flag_i = i_flag[FW-1];
assign cont_flag_enable = start;
...
assign valid_i = (busy_o | o_v) & ~o_e_int;
assign o_v = valid_o;

// End reg
assign end_enable = ~o_v | (o_v & o_r);
assign end_i = (cntcl_o=={ {SW-7 {1'b0}}, 1'b1 });
assign o_e_int = end_o;
assign o_e = o_e_int & ~cont_flag_o;

// Busy reg
assign busy_enable = start | (o_v & o_r & o_e_int);
assign busy_i = start;
assign i_r = ~busy_o;
...
// Capture flag reg
zcsl_dffea #(.WIDTH(1)) ICONT_FLAG(.clk(clk), .reset(reset), .enable(cont_flag_enable), .i(cont_flag_i), .o(cont_flag_o));
...
always@(posedge(clk)) begin
	...
	if(o_v & o_r & o_e_int) begin
		int_ends = int_ends+1;
		$display("@%d: end flag %d seen", $time, int_ends);
	end
	
	if(start) begin
		int_start = int_start+1;
		$display("@%d: start wed no. %d", $time, int_start);
	end
	
	if(cont_flag_enable & cont_flag_i) begin
		//$display("@%d: Last wed seen", $time);
	end
	
	if(o_e & o_v & o_r) begin
		$display("@%d: Last address generated", $time);
	end
