...
if (WIDTH == 1)
	...
	for (m = 0; m < WIDTH; m = m + 1) begin : u
		wire [0:WAYS-1] mux_in;
		for (n = 0; n < WAYS; n = n + 1) begin : v
			assign mux_in[n] = i[(n*WIDTH)+m];
		end
		zcsl_selector #(.WAYS(WAYS), .WIDTH(1)) IMUX(.sel(sel), .i(mux_in), .o(o[m]));
	end
