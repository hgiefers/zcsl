...
module zcsl_shrea
#(
	parameter WIDTH = 1, parameter LENGTH=64
)
(
	input clk,
	input reset,
	...
	reg [0:WIDTH*LENGTH-1] sr = {WIDTH*LENGTH {1'b0}};

	always@(posedge(clk) or posedge(reset)) begin
		if(reset)
			sr <= {WIDTH*LENGTH {1'b0}};
		else begin
			if(enable) begin
					sr[WIDTH:WIDTH*LENGTH-1] <= sr[0:WIDTH*(LENGTH-1)-1];
					sr[0:WIDTH-1] <= i;
			end
		end
	end

	assign o = sr[(LENGTH-1)*WIDTH:LENGTH*WIDTH-1];
